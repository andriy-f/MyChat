...
namespace MyChatServer.Crypto
{
	...
	public class RSA 
    {
        ...
        public static RSA_Secret_Key extractSecretKey(BigInteger[][] biKey)
        {
            RSA_Secret_Key sk;
        ...
        public static RSA_Public_Key extractPublicKey(BigInteger[][] biKey)
        {
            RSA_Public_Key pk;
		...
		public static BigInteger[][] ParseSecretKey(RSA_Secret_Key rskKey)
        {
		...
		public static BigInteger[][] Generate(int nbits)
        {
			...
			*/
            ...
            //if (!CheckKey(sk)) throw (new Exception("Generator failed! :)"));
		...
		//:( wrong
		public static bool CheckKey(RSA_Secret_Key sk) 
        {
			...
			// - pInv * p (mod q) = 1
			if (((sk.p.modInverse(sk.q) * sk.p) % sk.q) != 1)
				return false;
		...
		/// be encrypted</param>
        /// <param name="rpkKey">The public key packet with the key
		...
		/// <remarks>No remarks.</remarks>
		public static BigInteger[] Encrypt(BigInteger biPlain, RSA_Public_Key rpkKey) 
        {
			if ((rpkKey.e == 0) || (rpkKey.n == 0))
				throw new System.ArgumentException("This is not a valid public key");
            //return biPlain.modPow(rpkKey.e, rpkKey.n);
		...
		/// be decrypted</param>
        /// <param name="skey">The secret key packet with the key
		/// material for the decryption</param>		
		/// <returns>The decrypted ciphertext.</returns>
		/// <remarks>No remarks.</remarks>
		public static BigInteger Decrypt(BigInteger[] biCipher, RSA_Secret_Key skey) 
        {			
			//if (!CheckKey(skey)) throw(new Exception("This key does not fullfill the requirements of a valid RSA key. Please check if someone messed with your keys!"));

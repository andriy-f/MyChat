using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

using Org.BouncyCastle.Crypto.Agreement;
using Org.BouncyCastle.Crypto.Engines;
using Org.BouncyCastle.Crypto.Generators;
using Org.BouncyCastle.Crypto.Parameters;
using Org.BouncyCastle.Math;
using Org.BouncyCastle.Security;
using Org.BouncyCastle.Crypto;

namespace CustomCrypto
{
    public class ECDHWrapper
    {
        #region Fields

        private static readonly BigInteger g512 = new BigInteger("153d5d6172adb43045b68ae8e1de1070b6137005686d29d3d73a7749199681ee5b212c9b96bfdcfa5b20cd5e3fd2044895d609cf9b410b7a0f12ca1cb9a428cc", 16);
        private static readonly BigInteger p512 = new BigInteger("9494fec095f3b85ee286542b3836fc81a5dd0a0349b4c239dd38744d488cf8e31db8bcb7d33b41abb9e5a33cca9144b1cef332c94bf0573bf047a3aca98cdf3b", 16);

        private static readonly BigInteger g768 = new BigInteger("7c240073c1316c621df461b71ebb0cdcc90a6e5527e5e126633d131f87461c4dc4afc60c2cb0f053b6758871489a69613e2a8b4c8acde23954c08c81cbd36132cfd64d69e4ed9f8e51ed6e516297206672d5c0a69135df0a5dcf010d289a9ca1", 16);
        private static readonly BigInteger p768 = new BigInteger("8c9dd223debed1b80103b8b309715be009d48860ed5ae9b9d5d8159508efd802e3ad4501a7f7e1cfec78844489148cd72da24b21eddd01aa624291c48393e277cfc529e37075eccef957f3616f962d15b44aeab4039d01b817fde9eaa12fd73f", 16);

        private static readonly BigInteger g1024 = new BigInteger("", 16);
        private static readonly BigInteger p1024 = new BigInteger("", 16);

        int bits;

        DHPublicKeyParameters pu1;
        DHPrivateKeyParameters pv1;

        DHAgreement e1;
        BigInteger m1;

        byte[] pubdata;

        #endregion

        public ECDHWrapper(int newbits)
        {
            DHKeyPairGenerator kpGen;
            
            switch (newbits)
            {
                case 512:
                    kpGen = getDHKeyPairGenerator(g512, p512);
                    //p = p512; g=g512;
                    break;
                case 768:
                    kpGen = getDHKeyPairGenerator(g768, p768);
                    //p = p768; g=g768;
                    break;
                case 1024:
                    kpGen = getDHKeyPairGenerator(g1024, p1024);
                    break;
                default:
                    throw new ArgumentException("newbits must be 512, 768 or 1024");                    
            }

            bits = newbits;

            AsymmetricCipherKeyPair pair = kpGen.GenerateKeyPair();

            pu1 = (DHPublicKeyParameters)pair.Public;
            pv1 = (DHPrivateKeyParameters)pair.Private;

            e1 = new DHAgreement();
            e1.Init(new ParametersWithRandom(pv1, new SecureRandom()));
            m1 = e1.CalculateMessage();

            pubdata = getPubData();
        }

        

        private Byte[] getPubData()
        {
            int hdrSize = 8;
            Byte[] bY = pu1.Y.ToByteArray();
            Byte[] bm1 = m1.ToByteArray();
            int dataSize = bY.Length + bm1.Length;
            //Header - 4+4       
            Byte[] msg = new Byte[hdrSize + dataSize];            
            BitConverter.GetBytes(bY.Length).CopyTo(msg, 0);
            BitConverter.GetBytes(bm1.Length).CopyTo(msg, 4);         
            //data
            int itr = hdrSize;
            bY.CopyTo(msg, itr);
            itr += bY.Length;
            bm1.CopyTo(msg, itr);
            return msg;
        }

        public byte[] calcAgreement(Byte[] pubdata)
        {
            return calcAgreement(pubdata, bits / 8);
        }

        public byte[] calcAgreement(Byte[] pubdata, int rbytes)//rbytes - return length
        {
            //int hdrSize =8
            //Reading header
            int bY2Size = BitConverter.ToInt32(pubdata, 0);
            int bm2Size = BitConverter.ToInt32(pubdata, 4);
            
            //Reading data
            
            int itr = 8;

            BigInteger Y2 = new BigInteger(pubdata, itr, bY2Size);
            itr += bY2Size;

            BigInteger m2 = new BigInteger(pubdata, itr, bm2Size);

            DHParameters newDHParameters;
            switch (bits)
            {
                case 512:
                    newDHParameters=new DHParameters(p512, g512);
                    break;
                case 768:
                    newDHParameters = new DHParameters(p768, g768);
                    break;
                case 1024:
                    newDHParameters = new DHParameters(p1024, g1024);
                    break;
                default:
                    throw new ArgumentException("bits must be 512, 768 or 1024");                    
            }
            
            DHPublicKeyParameters pu2 = new DHPublicKeyParameters(Y2, newDHParameters);

            byte[] agr=e1.CalculateAgreement(pu2, m2).ToByteArray();//Variable length (not must be bits / 8
            if (agr.Length == rbytes)
                return agr;
            else
            {                
                byte[] newres = new byte[rbytes];
                int lessLen = Math.Min(agr.Length, rbytes);
                for (int i = 0; i < lessLen; i++)
                    newres[i] = agr[i];
                for (int i = lessLen; i < rbytes; i++)
                    newres[i] = 0;
                return newres;
            }
        }

        public byte[] PubData
        {
            get { return pubdata; }
        }

        private DHKeyPairGenerator getDHKeyPairGenerator(BigInteger g, BigInteger p)
        {
            DHParameters dhParams = new DHParameters(p, g);
            DHKeyGenerationParameters dhkgParams = new DHKeyGenerationParameters(new SecureRandom(), dhParams);
            DHKeyPairGenerator kpGen = new DHKeyPairGenerator();

            kpGen.Init(dhkgParams);

            return kpGen;
        }
    }
}
